...
namespace PSXEmulator.Core.x64_Recompiler {
    public unsafe class NativeMemoryManager : IDisposable {
        ...
        private List<(ulong address, int size)> InvalidBlocks = new List<(ulong address, int size)>();
        ...
        private NativeMemoryManager() {
            //Allocate 32MB of executable memory
            ...
            Console.WriteLine("[NativeMemoryManager] Allocated!");
        }
